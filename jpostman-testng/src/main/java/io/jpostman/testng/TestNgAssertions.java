		...
		for (int i = 0; i < values.size(); i++) {
			...
			assertWithLog(() -> {
				if (!type.isInstance(value)) {
					Assert.fail(format("Path value has wrong type. Expected: {}, Value: {}, Index: {}",
							type.getSimpleName(), value, index));
				}

				V typedValue = type.cast(value);
				if (!predicate.test(typedValue, index)) {
					Assert.fail(valueIndexMessage(message(message, "Path value did not match"), typedValue, index));
				}
			});
		}
