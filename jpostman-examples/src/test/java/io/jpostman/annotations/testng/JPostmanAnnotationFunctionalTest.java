...
public class JPostmanAnnotationFunctionalTest {
	...
	private static final Logger log = LoggerFactory.getLogger(JPostmanAnnotationFunctionalTest.class);
	...
	public void testExecutionFailedSoftVerifyFixture() throws Exception {
		...
		runner.run(fixture, method);
		AssertionError error = expectThrows(AssertionError.class, () -> invokeTestMethod(fixture, method));
		assertEquals(debug(error.getMessage()), "Status code mismatch: expected [200] but found [401]\n",
				"Actual message: " + error.getMessage());
	}
